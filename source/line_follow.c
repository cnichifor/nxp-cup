#include "line_follow.h"

// calculating the line to follow
line get_line(uint16_t vectors[], size_t num_vectors)
{
	line l;
	l.x0 = 0;
	l.y0 = 0;
	l.x1 = 0;
	l.y1 = 0;

	if(num_vectors >= 2)
	{
		l.x0 = 40;
		l.y0 = 26;
//		l.x0 = (vectors[0] + vectors[4 + 0]) / 2;
//		l.y0 = (vectors[1] + vectors[4 + 1]) / 2;
		l.x1 = (vectors[2] + vectors[4 + 2]) / 2;
		l.y1 = (vectors[3] + vectors[4 + 3]) / 2;
	}

	return l;
}

// get angle (radians)
double angle(line l)
{
    double dx = (double)l.x1 - (double)l.x0;   // dreapta = +
    double dy = (double)l.y0 - (double)l.y1;   // sus/inainte = +

    double a = atan2(dx, dy) * 60;   // nu atan2(dy, dx)

    if (a > STEERING_LIMIT_RIGHT)
        a = STEERING_LIMIT_RIGHT;
    else if (a < STEERING_LIMIT_LEFT)
        a = STEERING_LIMIT_LEFT;

    return a;
}

// moving trough the line
void move(line l)
{
    double steer_angle = angle(l) + STEERING_OFFSET;

    printf("steer_x1000=%d\n", (int)(steer_angle));

    Steer(steer_angle);
    HbridgeSpeed(&g_hbridge, 25, 25);
}
